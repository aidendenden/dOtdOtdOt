...
public class Move_3_0 : MonoBehaviour {
	...
	public bool bringToFrontOnOver = false;				// Determines if this object will be set as the last sibling in the hierarchy when the cursor is on ths object
	...
	void MoveObject()				// This function is used to determine the amount of movement of the cursor, and then it moves the object
	{
		...
		if (xOnly == false && yOnly == false)
		{
			...
			{
				transform.position = new Vector2 (Input.mousePosition.x - offsetX, Input.mousePosition.y - offsetY);				// Moves the object along the x-axis and y-axis
			}
			else if (makeMoveHorizontal == true && makeMoveVertical == false)
			{
				transform.position = new Vector2 (Input.mousePosition.x - offsetX, transform.position.y);							// Moves the object along the x-axis
			}
			else if (makeMoveHorizontal == false && makeMoveVertical == true)
			{
				transform.position = new Vector2 (transform.position.x, Input.mousePosition.y - offsetY);							// Moves the object along the y-axis
			}
		}
