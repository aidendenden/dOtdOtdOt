...
public class MoveOther_1_0 : MonoBehaviour {
	...
	public bool disableMoveOtherObject = false;	// Determines if the ability to move the other object is enabled or disabled
	...
	void Update ()
	{
		if (disableMoveOtherObject == false)
		{
	...
	void PerformRaycast ()				// This function performs a raycast to detect if this object or its child is in front of other objects if any overlap at the cursor's position when the mouse button is initially pressed. If so, the other object is set as the last sibling in the hierarchy
	{
		...
		if(objectsHit[0].gameObject == this.gameObject || objectsHit[0].gameObject.transform.IsChildOf (transform))				// This section runs only if this object or its child is the front object where the cursor is when the mouse button is initially pressed
		{
	...
	public void MoveObject()										// This function is used to determine the amount of movement of the cursor, and then it moves the object
	{
		...
		otherObjectTransform.position = new Vector2 (Input.mousePosition.x - offsetX, Input.mousePosition.y - offsetY);			// Moves the other object
	}
	...
	public void MoveOtherObjectToggle ()			// This function toggles the ability to move the other object on and off. Good for events that toggle
	{
		if(disableMoveOtherObject == true)
		{
			disableMoveOtherObject = false;
		}
		else
		{
			disableMoveOtherObject = true;
		}	
	}
	
	
	public void DisableMoveOtherObject ()			// This function disables this ability to move the other object. Good for events that do not toggle
	{
		...
		else if(disableMoveOtherObject == false)
		{
			disableMoveOtherObject = true;
		}
	}
	...
	public void EnableMoveOtherObject ()			// This function enables the ability to move the other object. Good for events that do not toggle
	{
		if (disableMoveOtherObject == true)
		{
			disableMoveOtherObject = false;
