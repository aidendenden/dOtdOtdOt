...
public class ResizeParent_1_0 : MonoBehaviour {
	...
	public float borderThickness = 10;			// Sets border thickness of the parent
	public float minimumWidth = 0;				// Sets the minimum width the parent may resize to. The minimum width shall be set to no less than twice the thickness of the border
	public float minimumHeight = 0;				// Sets the minimum height the parent may resize to. The minimum height shall be set to no less than twice the thickness of the border
	...
	void Update ()
	{
		...
		{
			if (minimumWidth < borderThickness)									// This section sets the minimum width and height to twice the thickness of the border if they are less. This is so the parent cannot be resized into nothingness unless the border is deliberately set to 0
			{
	...
	void InitialResizeParentObject ()			// This function is used to determine if the cursor is over the object when the mouse button it initially pressed. It is also used to determine which direction(s) the object will be assigned to resize the parent
	{
		...
		{
			if (TopRight == true)						// This section determines which direction(s) the object will be assigned to resize the parent
			{
				sector = 1;
			}
	...
	void ResizeParentObject ()			// This function is used to determine the amount of movement of the cursor, if the minimum limits have been reached, and then it resizes the parent
	{
		...
		// The following section is broken down into each sector. Each sector determines the direction(s) that the parent will be resized. Each section has two purposes.
		// First, if the user continues to drag the cursor beyond the minimum limitations, the cursor will leave the draggable object.
		// While the parent will not minimize any further, if the cursor reverses direction, the parent would immediately enlarge even though the cursor may be nowhere near it.
		// These checks assure that the parent will no longer begin to enlarge until the cursor returns near the object on the axis of movement.
		// Secondly, the parent is resized based on the direction(s) assigned to the object. As for the offsets, the corresponding sides are as follows...
		// offsetMin.x : Left
		// offsetMin.y : Bottom
		// offsetMax.x : Right
		// offsetMax.y : Top
		...
		if(sector == 1)					// TopRight
		{
			if(TopRight == true)
			{
				...
				parentRectTransform.offsetMax = new Vector2 (parentRectTransform.offsetMax.x + movementX, parentRectTransform.offsetMax.y + movementY);						// Resizes the parent based on the appropriate offsets
			}
		}
		else if(sector == 2)			// BottomRight
		{
			if(BottomRight == true)
			{
				...
				parentRectTransform.offsetMin = new Vector2 (parentRectTransform.offsetMin.x, parentRectTransform.offsetMin.y + movementY);
		...
		else if(sector == 4)			// TopLeft
		{
			if(TopLeft == true)
			{
				...
				parentRectTransform.offsetMin = new Vector2 (parentRectTransform.offsetMin.x + movementX, parentRectTransform.offsetMin.y);
		...
		else if(sector == 5)			// BottomLeft
		{
			if(BottomLeft == true)
			{
				...
				parentRectTransform.offsetMin = new Vector2 (parentRectTransform.offsetMin.x + movementX, parentRectTransform.offsetMin.y + movementY);
			}
		}
