    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float GetNormalDiff(float3 center, float3 side)
            {
                ...
                normalDot = clamp01(smoothstep(0.0, 0.001, normalDot));
            ...
            float linearizeDepth(float rawDepth)
            {
                float near = 0.1;
                float far = 1;
                return (2.0 * near) / (far + near - rawDepth * (far - near));
            }
            ...
            float4 Frag (Varyings i) : SV_Target
            {
                ...
                sobel = smoothstep(0.001, 1, sobel);
                ...
                int k = 1; // 0 is no debug
                if (_DebugEffect == k++) return float4(concave, convex, outline, 1.0);
                if (_DebugEffect == k++) return float4(p.center.depth, p.center.depth, p.center.depth, 1.0);
                if (_DebugEffect == k++) return float4(p.center.normal, 1.0);
                if (_DebugEffect == k++) return float4(sobel, sobel, sobel, 1.0);
                if (_DebugEffect == k++) return float4(depthDiff, depthDiff, depthDiff, 1.0);
                if (_DebugEffect == k++) return float4(normalDiff, normalDiff, normalDiff, 1.0);
                if (_DebugEffect == k++) return float4(concaveNormalDiff, concaveNormalDiff, concaveNormalDiff, 1.0);
                if (_DebugEffect == k++) return float4(originalColor.rgb, 0);
